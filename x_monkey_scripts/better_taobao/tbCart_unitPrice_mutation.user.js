...
// @version     2022.10.26
// @description 在淘宝详情页自动计算单价
// @icon        https://www.taobao.com/favicon.ico
// @author      Leizingyiu
// @include     http*://cart.taobao.com/cart.htm*
// @grant       GM_setValue
// @grant       GM_getValue
// @license     GNU AGPLv3 
// 
...
const checking_local_reagent = "___",
    ...
    default_words = "磅 包 条 个 袋 杯 枚 颗 罐 公斤 斤 两 盒 桶";
...
function tbCart_unitPrice() {
    ...
    var selector = ".item-list .item-holder";
    // switch (true) {
    //     case isHomePage:
    ...
    [...document.querySelectorAll(selector)]
        .filter((dom) => (dom.hasAttribute("yiu_price_calculated") == false) || dom.getAttribute('yiu_price_calculated') != 'true')
        .map(function (dom) {


            console.log(dom, dom.getAttribute("yiu_price_calculated"), dom.getAttribute('data-timestamp'));

            let t = new Date();
            dom.setAttribute('data-timestamp', t);


            var priceDom = dom.querySelector(".td-sum em"),
                price = priceDom.innerText.match(/[\d\.]+/g);
            console.log(priceDom, price);

            var pre_text = dom.querySelector('.td-info').innerText,
                text = [gramReg, volReg, wordsReg].filter(reg => pre_text.match(reg)).length ?
                    pre_text :
                    dom.querySelector('.td-item').innerText,
            ...
            var unit = "",
                ...
                priceU;
            let stepConsole = () => {
                console.log(dom, price, priceText, priceKg, priceL, priceU);
            }
