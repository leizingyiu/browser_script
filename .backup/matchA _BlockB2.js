...
var block_keywords = '的，好，转发，美女，图片，视频，转发，分享，肖战，双立人，滴露，益达，usmile,gucci,GUCCI,开小灶，隅田川，德芙，石头科技，欧扎克'.split(/[\s,，]{1,}/).filter(a => Boolean(a)),
    ...
    webSelectors = {
        ...
        'weibo.com': {
            '.vue-recycle-scroller__item-view': {//微博 全部关注 
                'detect': 'article',
                'hide': 'div[class*=content_row],div[class*=Feed_retweet],div.picture,'
            }
        }
    };
// console.log(wordsReg);
function blockThem() {
    console.log('blockThem');
    let checker = 'block-checked';
    console.log(webSelectors, Object.keys(webSelectors).filter(host => window.location.host.indexOf(host) != -1));

    Object.keys(webSelectors).filter(host => window.location.host.indexOf(host) != -1).map(host => {
        ...
        Object.keys(selectors).map(selector => {
            ...
            [...document.querySelectorAll(selector)].filter(dom => !dom.hasAttribute(checker)).map(dom => {
                ...
                let detectDom = dom.querySelector(selectors[selector]['detect']),
                    hasHideDom = dom.querySelector(selectors[selector]['hide']);

                if (detectDom) {
                    console.log(dom, detectDom, detectDom.innerText, detectDom.innerHTML, hasHideDom);
                }
                ...
                if (detectDom && detectDom.innerHTML.match(wordsReg)) {
                    ...
                    if (hasHideDom) {
                        let hideDoms = dom.querySelectorAll(selectors[selector]['hide']);
